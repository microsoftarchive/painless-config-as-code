...
function configurePackageEnvironments(providers, environmentModules, environment) {
  for (let i = 0; i < environmentModules.length; i++) {
    ...
    const npmName = environmentModules[i];
    ...
    if (typeof(environmentPackage) === 'function') {
      ...
      providers.push(objectProvider(environmentPackage));
...
function initialize(options) {
  ...
  if (provider.testConfiguration) {
    ...
    if (pkg && pkg.painlessConfigEnvironments && Array.isArray(pkg.painlessConfigEnvironments)) {
      environmentModules.push(...pkg.painlessConfigEnvironments);
    } else if (pkg && pkg.painlessConfigEnvironments && pkg.painlessConfigEnvironments.split) {
      const definedPackages = pkg.painlessConfigEnvironments.split(',');
      environmentModules.push(...definedPackages);
    } else {
